...
const app = createApp({
  setup() {
    ...
    async function handleLoginWrapped() {
      ...
      confirm.resetEvaluationDismissed();
      confirm.scheduleEvaluationReminder();
    }

    function isPreviewPeriod() {
      const turn = auth.currentTurn.value;
      if (!turn) return false;
      const now = timeline.mockNow.value;
      const selectBegin = new Date(turn.beginTime.replace(' ', 'T'));
    ...
    return {
      ...
      onTimetableCellClick(weekday, start, end) {
        if (isPreviewPeriod()) {
          ElMessage.warning('选课未开始');
          return;
        }
        course.onTimetableCellClick(weekday, start, end);
      },
      ...
      // v2: Evaluation check (HTA 1.4)
      evaluationCompleted: confirm.evaluationCompleted,
      showEvaluationBanner: confirm.showEvaluationBanner,
      dismissEvaluationBanner: confirm.dismissEvaluationBanner,
      goToEvaluation: confirm.goToEvaluation,
