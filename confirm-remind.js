/**
 * confirm-remind.js - Graduate preselection confirmation reminder (P0-5) & Evaluation check (HTA 1.4)
 */
...
function useConfirmRemind(loadJSON, currentStudent) {
  ...
  const evaluationDismissed = ref(false);
  ...
  const showEvaluationBanner = computed(() => {
    return !evaluationCompleted.value && !evaluationDismissed.value;
  });

  function dismissEvaluationBanner() {
    evaluationDismissed.value = true;
  }

  function resetEvaluationDismissed() {
    evaluationDismissed.value = false;
  }


  function scheduleEvaluationReminder() {
    if (evaluationCompleted.value) return;
    setTimeout(() => {
        ElMessage({
          type: 'warning',
  ...
  return {
    ...
    // Evaluation check (HTA 1.4)
    evaluationCompleted, showEvaluationBanner,
    dismissEvaluationBanner, resetEvaluationDismissed, scheduleEvaluationReminder, goToEvaluation
  };
}
